...
use App\Exports\ExportDataSales;
use Carbon\Carbon;
...
class DataSalesController extends Controller
{
    ...
    public function ajax(Request $request)
    {
        ...
        if (!empty($request->tanggal)) {
            $data_sales->where('tanggal', $request->source);
        }
        ...
        if (!empty($request->status_member)) {
            $data_sales->where('status_member', $request->recipient);
        }

        if (!empty($request->status_cek_is_member)) {
            $data_sales->where('status_cek_is_member', $request->recipient);
        }

        if (!empty($request->status_cek_poin)) {
            $data_sales->where('status_cek_poin', $request->recipient);
        }
    ...
    public function import(Request $request) 
    {
        ...
        // Excel::import(new ImportDataSales,$request->file('file'));
        Excel::import(new ImportDataSales,$path);
 
            
        return redirect('/data-sales')->with('success', 'The file has been excel imported to database ');
    }
